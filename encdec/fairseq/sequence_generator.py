...
class SequenceGenerator(object):
    def __init__(
        ...
        diverse_beam_groups=-1, diverse_beam_strength=0.5, desired_length=-1,
    ):
    ...
    def _generate(self, encoder_input, beam_size=None, maxlen=None, prefix_tokens=None):
        ...
        for model in self.models:
            ...
            encoder_input_without_target_length = {
                k: v for k, v in encoder_input.items()
                if k != 'target_length'
            }
            encoder_out = model.encoder(**encoder_input_without_target_length)
            ...
            target_length = encoder_input['target_length'].index_select(0, new_order)
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                for i, model in enumerate(self.models):
                    ...
                    target_lengths[i] = target_lengths[i].index_select(0, reorder_state)

            lprobs, avg_attn_scores = self._decode(tokens[:, :step + 1], encoder_outs, target_lengths, incremental_states)
    ...
    def _decode(self, tokens, encoder_outs, target_lengths, incremental_states):
        if len(self.models) == 1:
            return self._decode_one(tokens, self.models[0], encoder_outs[0], target_lengths[0], incremental_states, log_probs=True)

        log_probs = []
        avg_attn = None
        for index in range(len(self.models)):
            model = self.models[index]
            encoder_out = encoder_outs[index]
            target_length = target_lengths[index]
            probs, attn = self._decode_one(tokens, model, encoder_out, target_length, incremental_states, log_probs=True)
    ...
    def _decode_one(self, tokens, model, encoder_out, target_length, incremental_states, log_probs):
        with torch.no_grad():
            if incremental_states[model] is not None:
                decoder_out = list(model.decoder(tokens, target_length, encoder_out, incremental_state=incremental_states[model]))
            else:
                decoder_out = list(model.decoder(tokens, target_length, encoder_out))
